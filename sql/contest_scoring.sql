...
-- ============================================================
-- 3. SAMPS DEMO POINTS: +2 pts per completed SAMPS specialist interaction
--    Two fixes applied:
--    (a) Attribution: Escalation-based (AE who filed DSR) + account-owner fallback.
--        Old approach used account ownership only, missing ~75% of demos.
--    (b) Activity filter: CALL_ANSWERED is NULL for ~80% of non-Call activities
--        (Strategy Syncs, "Other" type DSR consultations). These are real
--        interactions (avg 47 min, 98% DM-worked) that were being dropped.
--        Now includes: CALL_ANSWERED=1 OR (NULL + real activity type + duration>0).
-- ============================================================
samps_demos AS (
    ...
    WHERE (sa.CALL_ANSWERED = 1
           OR (sa.CALL_ANSWERED IS NULL
               AND sa.ACTIVITY_TYPE_GROUPED IN ('Call', 'Other', 'Strategy Sync (Google Meet)', 'Strategy Sync (Slack)')
               AND sa.CALL_DURATION_MINUTES > 0))
      AND sa.ACTIVITY_DATE >= '2026-08-31'
      AND sa.ACTIVITY_DATE <= '2026-09-30'
      AND esc.CREATED_DATE >= '2026-07-01'
    UNION
    -- Method 2: Account-owner fallback (catches demos where no escalation exists)
    SELECT DISTINCT
        r.FULL_NAME,
        sa.TASK_ID,
        sa.ACTIVITY_DATE,
    ...
    INNER JOIN APP_SALES.APP_SALES_ETL.SFDC_OWNERS so
        ON acct.OWNER_ID = so.OWNER_ID
    INNER JOIN roster r
